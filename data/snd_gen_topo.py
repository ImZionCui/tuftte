...
with open(demand_matrices_dir + '/' + dms_input_files[0]) as f:
    ...
    while not line.startswith(")"):
        ...
        # Node de1 and node de2 are considered as one node.
...
os.mkdir(output_dir)
os.mkdir(output_dir + "/opts_A")
os.mkdir(output_dir + "/opts_D")
...
with open(output_dir + '/' + pathlib.PurePath(output_dir).name + "_int.pickle.nnet", "w", newline='\n') as f:
    for e in edges_list:
        f.write(str(e[0]) + ',' + str(e[1]) + ',' + e[2] + ',' + str(weibull(0.8)*1e-4) + '\n')
...
count = 0
for file in dms_input_files:
    assert file.endswith('.txt')
    # There is something wrong in this file
    if file.endswith('20050527-1745.txt'):
        continue
    is_empty = True
